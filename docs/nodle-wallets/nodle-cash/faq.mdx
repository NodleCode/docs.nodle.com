...
### Can I expect to earn more with the Hotspot app?

There is supposed to be no difference between a hotspot and a smartphone. The Nodle Network will treat them exactly the same 
way, in other words, all nodes are equals if they have the same capabilities. 

As a matter of fact, the Nodle Network rewards nodes for their availability and their location. 
- All things being equal, if two nodes are in the same location, the one that works longer available will earn more.
- All things being equal, if two nodes are available for an hour, the one that covers the more distance, will earn more.

However, as the Hotspot app is aimed to be used on devices that are plugged in and online 24/7, it is likely that the
availability will be greater than on the main phone, and therefore the earnings should be more stable and consistent over time. 

You can also safely place the Hotspot device far away from you and from your other devices to create wider Nodle Network 
coverage and increase your rewards. Nobody will be able to steal your NODL tokens as they can only be collected from the 
main Nodle app.
